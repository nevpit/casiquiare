...
def test_imports():
    ...
    raster_path = Path(__file__).resolve().parents[1] / "io_utils" / "raster.py"
    raster_spec = importlib.util.spec_from_file_location("io_utils.raster", raster_path)
    ...
    sys.modules["io_utils.lidar"] = module

    # Import io_utils.data_paths explicitly from file to avoid stdlib conflict
    paths_path = Path(__file__).resolve().parents[1] / "io_utils" / "data_paths.py"
    spec_paths = importlib.util.spec_from_file_location("io_utils.data_paths", paths_path)
    ...
    sys.modules["io_utils.data_paths"] = paths_module

    importlib.import_module("io_utils.lidar")
    importlib.import_module("io_utils.data_paths")
