...
def test_scan_cli_basic(caplog):
    with (
        mock.patch("agents.eyes.cli.load_raster", return_value=(np.zeros((1, 1)), None, None)),
        mock.patch("agents.eyes.cli.scan_area") as scan,
    ):
        scan.return_value = []
        with caplog.at_level("INFO", logger="agents.eyes.cli"):
            cli.main(["scan", "area.tif"])
        scan.assert_called_with(np.zeros((1, 1)), None)
