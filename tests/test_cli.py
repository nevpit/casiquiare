...
def test_scan_cli_basic(caplog):
    with mock.patch("eyes.cli.scan_area") as scan:
        scan.return_value = []
        with caplog.at_level("INFO", logger="eyes.cli"):
...
def test_scan_cli_save_options(tmp_path):
    ...
    with (
        mock.patch("eyes.cli.scan_area", return_value=features) as scan,
        mock.patch("eyes.cli.save_geojson") as save_geo,
        mock.patch(
            "io_utils.raster.load_raster",
            return_value=(np.zeros((1, 1)), None, None),
        ),
        mock.patch("eyes.tools.save_snippets", return_value=["area_snippets/snippet_0.png"]) as save_snip,
    ):
